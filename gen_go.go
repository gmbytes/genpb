...
func GenerateGo(cfg *Config) error {
	// Parse proto files
	// enumMap is keyed by enum name (e.g. "ReqLogin", "RspLogin", "DspLogin")
...
func writeGoCmdExt(path string, enumMap map[string]EKeyInfo, reqMessages, rspMessages, dspMessages []string) error {
	...
	// message interface (unexported, embeds proto.Message)
	buf.WriteString("type message interface {\n")
	buf.WriteString("\tproto.Message\n\n")
	buf.WriteString("\tKey() EKey_T\n")
	buf.WriteString("}\n\n")

	// Package struct: wraps message with error code and cached bytes
	buf.WriteString("type Package struct {\n")
	buf.WriteString("\tmsg   message\n")
	buf.WriteString("\terr   EErrorCode_T\n")
	buf.WriteString("\tbytes []byte\n")
	buf.WriteString("}\n\n")

	buf.WriteString("func NewPackage(msg proto.Message, errs ...EErrorCode_T) *Package {\n")
	buf.WriteString("\terr := EErrorCode_Ok\n")
	buf.WriteString("\tif len(errs) != 0 {\n")
	buf.WriteString("\t\terr = errs[0]\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn &Package{\n")
	buf.WriteString("\t\tmsg: msg.(message),\n")
	buf.WriteString("\t\terr: err,\n")
	buf.WriteString("\t}\n")
	buf.WriteString("}\n\n")

	buf.WriteString("func (p *Package) Key() EKey_T {\n")
	buf.WriteString("\treturn p.msg.Key()\n")
	buf.WriteString("}\n\n")

	// Package.Marshal: [cmd 2B] [errCode 2B] [bodyLen 4B] [body NB]
	buf.WriteString("func (p *Package) Marshal() ([]byte, error) {\n")
	buf.WriteString("\tif len(p.bytes) != 0 {\n")
	buf.WriteString("\t\treturn p.bytes, nil\n")
	buf.WriteString("\t}\n\n")
	buf.WriteString("\tif p.msg == nil {\n")
	buf.WriteString("\t\treturn nil, errors.New(\"message is nil\")\n")
	buf.WriteString("\t}\n\n")
	buf.WriteString("\tbytes := make([]byte, 4, 8)\n\n")
	buf.WriteString("\tbinary.LittleEndian.PutUint16(bytes[0:], uint16(p.msg.Key()))\n")
	buf.WriteString("\tbinary.LittleEndian.PutUint16(bytes[2:], uint16(p.err))\n\n")
	buf.WriteString("\tif p.err != EErrorCode_Ok {\n")
	buf.WriteString("\t\treturn bytes, nil\n")
	buf.WriteString("\t}\n\n")
	buf.WriteString("\tbody, marshalErr := proto.Marshal(p.msg)\n")
	buf.WriteString("\tif marshalErr != nil {\n")
	buf.WriteString("\t\treturn nil, marshalErr\n")
	buf.WriteString("\t}\n\n")
	...
	// Register all message types: enum name == message name (Req/Rsp/Dsp prefix included)
...
// Returns a map keyed by enum name (e.g. "ReqLogin", "DspLogin").
func parseEKeyEnum(path string) (map[string]EKeyInfo, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}

	result := make(map[string]EKeyInfo)
	...
	for _, match := range matches {
		if len(match) >= 3 {
			...
			result[name] = EKeyInfo{Name: name, Value: value}
		}
	}
