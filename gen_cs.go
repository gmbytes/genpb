...
func parseCSEKeyEnum(path string) (map[string]CSEnumInfo, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}

	result := make(map[string]CSEnumInfo)
	...
	for _, match := range matches {
		if len(match) >= 3 {
			...
			result[name] = CSEnumInfo{Name: name, Value: value}
		}
	}
...
func writeCSCmd(path string, enumMap map[string]CSEnumInfo, reqMessages, rspMessages, dspMessages []string) error {
	...
	// Collect and sort enum entries by value
	...
	for _, info := range enumMap {
		enumValues = append(enumValues, EnumEntry{value: info.Value, info: info})
	}
